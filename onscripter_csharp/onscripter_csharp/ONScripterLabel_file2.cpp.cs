...
//		#include "ONScripterLabel.h"
		...
		public partial class ONScripterLabel {
			public int loadSaveFile2( int file_version, bool input_flag )
			{
			//printf("loading file, version %d\n", file_version);
			    ...
			    if ( !input_flag ) {
			        ...
			        sentence_font.color[0] = (byte)(readInt() & 0xff);
			        sentence_font.color[1] = (byte)(readInt() & 0xff);
			        sentence_font.color[2] = (byte)(readInt() & 0xff);
			        cursor_info[0].remove();
			        readStr( ref cursor_info[0].image_name );
			        if ( null!=cursor_info[0].image_name ){
			            parseTaggedString( cursor_info[0] );
			            setupAnimationInfo( cursor_info[0] );
			            if ( null!=cursor_info[0].image_surface )
			                cursor_info[0 ].visible = true;
			        }
			        cursor_info[1].remove();
			        readStr( ref cursor_info[1].image_name );
			        if ( null!=cursor_info[1].image_name ){
			        	parseTaggedString( cursor_info[1] );
			        	setupAnimationInfo( cursor_info[1] );
			            if ( null!=cursor_info[1].image_surface )
			    ...
			    if ( !input_flag ) {
			        ...
			        readStr( ref window_effect.anim.image_name ); // probably
			    }
			    ...
			    if ( !input_flag ) {
			        ...
			        for ( i=0 ; i<3 ; i++ )
			        	sentence_font.window_color[2-i] = (byte)(readChar() & 0xff);
			        ...
			        if ( !sentence_font.is_transparent && null!=sentence_font_info.image_name ){
			    ...
			    if (num_nest > 0){
			    	file_io_buf_ptr = (uint)(file_io_buf_ptr + ((num_nest-1)*4));
			        while( num_nest > 0 ){
			            if ( !input_flag ) {
			                i = readInt();
			                if (i > 0){
			                ...
			                if (null!=root_nest_info.next) root_nest_info.next.previous = info;
			                root_nest_info.next = info;
			                info.previous = root_nest_info;
			            }
			        }
			        num_nest = readInt();
			        file_io_buf_ptr = (uint)(file_io_buf_ptr + (num_nest*4));
			    }
			    ...
			    if ( !input_flag ) {
			        ...
			        for ( i=0 ; i<3 ; i++ )
			        	monocro_color[2-i] = (byte)(readInt() & 0xff);
			        for ( i=0 ; i<256 ; i++ ){
			    ...
			    if ( !input_flag ) {
			        ...
			        readStr( ref seqmusic_file_name ); // MIDI file
			        readStr( ref wave_file_name ); // wave, waveloop
			        i = readInt();
			        ...
			        if ( null!=wave_file_name && wave_play_loop_flag )
			    ...
			    for ( i=0 ; i<MAX_PARAM_NUM ; i++ ){
			        ...
			        if ( input_flag && (j != 0) ){
			            ...
			            for ( j=0 ; j<3 ; j++ )
			            	bar_info[i].color[2-j] = (byte)(readChar() & 0xff);
			            readChar(); // 0x00
			    ...
			    for ( i=0 ; i<MAX_PARAM_NUM ; i++ ){
			        ...
			        if ( input_flag && null!=prnum_info[i] ){
			            ...
			            for ( j=0 ; j<3 ; j++ )
			            	prnum_info[i].color_list[0][2-j] = (byte)(readChar() & 0xff);
			            readChar(); // 0x00
			
			            char[] num_buf = new char[7];
			            script_h.getStringFromInteger( num_buf, prnum_info[i].param, 3, false, true );
			            setStr( ref prnum_info[i].file_name, num_buf );
			    ...
			    if ( !input_flag ) {
			        ...
			        btndef_info.remove();
			        readStr( ref btndef_info.image_name );
			        if ( null!=btndef_info.image_name && btndef_info.image_name[0] != '\0' ){
			    ...
			    if ( !input_flag ) {
			        ...
			        readStr( ref loop_bgm_name[0] );
			        readStr( ref loop_bgm_name[1] );
			        if ( null!=loop_bgm_name[0] ) {
			            if ( null!=loop_bgm_name[1] )
			    ...
			    if ( file_version >= 201 ){
			        if ( !input_flag ) {
			            ...
			            readStr( ref ruby_struct.font_name );
			    ...
			    if (file_version >= 204){
			        ...
			        if ( !input_flag ) {
			            ...
			            for ( i=0 ; i<MAX_SPRITE2_NUM ; i++ ){
			                ...
			                readStr( ref sprite2_info[i].image_name );
			                if ( null!=sprite2_info[i].image_name ){
			    ...
			    if ( !input_flag ) {
			        ...
			        for ( i=0 ; i<text_num ; i++ ){
			            ...
			            while( 0!=current_page.text[current_page.text_count++] );
			            if (file_version == 203) readChar(); // 0
			    ...
			    if ( !input_flag ) {
			        ...
			        CharPtr buf = script_h.getAddressByLine( i );
			...
			public void saveSaveFile2( bool output_flag )
			{
			    int i, j;
			    ...
			    for ( i=0 ; i<3 ; i++ )
			    	writeChar( (char)(sentence_font.window_color[2-i]), output_flag );
			    writeChar( (char)(( sentence_font.is_transparent )?0x00:0xff), output_flag );
			    ...
			    for ( i=0 ; i<MAX_PARAM_NUM ; i++ ){
			        if ( null!=bar_info[i] ){
			            ...
			            for ( j=0 ; j<3 ; j++ )
			            	writeChar( (char)bar_info[i].color[2-j], output_flag );
			            writeChar( (char)0x00, output_flag );
			        }
			        else{
			            writeInt( 0, output_flag );
			            writeInt( -1, output_flag );
			    ...
			    for ( i=0 ; i<MAX_PARAM_NUM ; i++ ){
			        if ( null!=prnum_info[i] ){
			            ...
			            for ( j=0 ; j<3 ; j++ )
			                writeChar( (char)prnum_info[i].color_list[0][2-j], output_flag );
			            writeChar( (char)0x00, output_flag );
			        }
			        else{
			            writeInt( 0, output_flag );
			    ...
			    writeInt( 0, output_flag );
			    writeChar( (char)((erase_text_window_mode==2)?1:0), output_flag );
			    ...
			    writeInt( 0, output_flag );
			    writeInt( 0, output_flag );
			    writeChar( (char)0, output_flag ); // added in version 205
			    ...
			    CharPtr buf = script_h.getAddressByLine( current_label_info.start_line + current_line );
