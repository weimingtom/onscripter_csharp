...
//		#include <cstdio>
//		#include <string.h>
		private const string NSA_ARCHIVE_NAME = "arc";
		private const string NSA_ARCHIVE_NAME2 = "arc%d";
		...
		public partial class NsaReader : SarReader
		{
			...
			{
				DirPaths paths = new DirPaths(nsa_path);
			...
			public int processArchives( DirPaths path )
			{
				int i,j,k,n,nd;
			    FILEPtr fp;
			    CharPtr archive_name = new char[256], archive_name2 = new char[256];
			
			    if ( 0==base.open( "arc.sar" ) ) {
			        sar_flag = true;
			    }
			    else {
			        sar_flag = false;
			...
			public override int getNumFiles(){
				int i;
				int total = (int)archive_info.num_of_files; // start with sar files, if any
			
				total += (int)archive_info_nsa.num_of_files; // add in the arc.nsa files
			
				for ( i=0 ; i<num_of_nsa_archives-1 ; i++ ) total += (int)archive_info2[i].num_of_files; // add in the arc?.nsa files
			
			    for ( i=0 ; i<num_of_ns2_archives ; i++ ) total += (int)archive_info_ns2[i].num_of_files; // add in the ##.ns2 files
			...
			#if !TOOLS_BUILD
			    // direct read
			    if ( 0!=( ret = DirectReader_getFileLength( file_name ) ) ) return ret;
			#endif
			    // ns2 read
			    for ( i=0 ; i<num_of_ns2_archives ; i++ ){
			        if ( 0!=(ret = getFileLengthSub( archive_info_ns2[i], file_name )) ) return ret;
			    }
			    
			    // nsa read
			    if ( 0!=( ret = getFileLengthSub( archive_info_nsa, file_name )) ) return ret;
			
			    // nsa? read
			    for ( i=0 ; i<num_of_nsa_archives-1 ; i++ ){
			        if ( 0!=(ret = getFileLengthSub( archive_info2[i], file_name )) ) return ret;
			    }
			    
			    // sar read
			    if ( sar_flag ) return this.getFileLength( file_name );
			...
			public override uint getFile( CharPtr file_name, UnsignedCharPtr buffer, ref int location, bool is_location_null=true )
			{
			    ...
			    // direct read
			    if ( 0!=( ret = DirectReader_getFile( file_name, buffer, ref location ) ) ) return ret;
			
			    // ns2 read
			    for ( int i=0 ; i<num_of_ns2_archives ; i++ ){
			        if ( 0!=(ret = getFileSub( archive_info_ns2[i], file_name, buffer )) ){
			            if ( !is_location_null ) location = ARCHIVE_TYPE_NS2;
			            return ret;
			        }
			    }
			
			    // nsa read
			    if ( 0!=(ret = getFileSub( archive_info_nsa, file_name, buffer )) ){
			        if ( !is_location_null ) location = ARCHIVE_TYPE_NSA;
			        return ret;
			    }
			    ...
			    for ( int i=0 ; i<num_of_nsa_archives-1 ; i++ ){
			        if ( 0!=(ret = getFileSub( archive_info2[i], file_name, buffer )) ){
			            if ( !is_location_null ) location = ARCHIVE_TYPE_NSA;
			            return ret;
			        }
			    }
			    ...
			    // sar read
			    if ( sar_flag ) return base.getFile( file_name, buffer, ref location, is_location_null );
